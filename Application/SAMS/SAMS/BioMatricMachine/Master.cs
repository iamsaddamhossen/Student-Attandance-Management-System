...
namespace BioMetrixCore
{
    public partial class Master : Form
    {
        ...
        public DataTable UserLogDataTable = new DataTable();
        private void SetUserDataLogColumns()
        {
            UserLogDataTable.Columns.Add("MachineNumber", typeof(string));
            UserLogDataTable.Columns.Add("IndRegID", typeof(string));
            UserLogDataTable.Columns.Add("DateTimeRecord", typeof(DateTime));
            UserLogDataTable.Columns.Add("DateOnlyRecord");
            UserLogDataTable.Columns.Add("TimeOnlyRecord", typeof(DateTime));
        }
        ...
        private void btnPullData_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (lstMachineInfo != null && lstMachineInfo.Count > 0)
                {
                    ...
                    {
                        foreach (MachineInfo record in lstMachineInfo)
                        {
                            UserLogDataTable.Rows.Add(record.MachineNumber, record.IndRegID, record.DateTimeRecord, record.DateOnlyRecord, record.TimeOnlyRecord);
                        }

                        bool status = DatabaseAccess.InsertAttendanceLog(UserLogDataTable);
                        if (status == true)
                        {
        ...
        private void btnImportDataFromDevicetoServer_Click(object sender, EventArgs e)
        {
            BioMetrixCore.Master frmMaster = new BioMetrixCore.Master();
            frmMaster.btnConnect_Click(sender, e);
            frmMaster.btnPullData_Click(sender, e);
